...
use whir_p3::ntt::{matrix::MatrixMut, transpose::transpose_square_swap_parallel};
...
fn benchmark_transpose_square_swap(c: &mut Criterion) {
    ...
    c.bench_function(&format!("transpose_square_swap_parallel {size}x{size}"), |b| {
        b.iter(|| {
            transpose_square_swap_parallel(black_box(view_a.clone()), black_box(view_b.clone()));
        });
    });
}
