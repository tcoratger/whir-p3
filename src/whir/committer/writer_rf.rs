    ...
    Challenger: FieldChallenger<F> + GrindingChallenger<Witness = F>,
{
        ...
        [F; DIGEST_ELEMS]: Serialize + for<'de> Deserialize<'de>,
    {
        ...
        for _ in 0..self.0.commitment_ood_samples {
            ...
            let point = MultilinearPoint::expand_from_univariate(point, self.num_variables);

            let eval =
                info_span!("ood evaluation").in_scope(|| polynomial.evaluate_hypercube(&point));
            proof.initial_ood_answers.push(eval);
            let flattened_eval = EF::flatten_to_base(vec![eval]);
            challenger.observe_slice(&flattened_eval);
...
mod tests {
    ...
    fn test_basic_commitment() {
        ...
        // Set up the DomainSeparator and initialize challenger
        ...
        let witness = committer.commit(&dft, &mut proof, &mut challenger, polynomial.clone());
    ...
    fn test_large_polynomial() {
        ...
        let dft = Radix2DFTSmallBatch::<F>::default();
        let committer = CommitmentWriter::new(&params);
        let _ = committer.commit(&dft, &mut proof, &mut challenger, polynomial);
    }
    ...
    fn test_commitment_without_ood_samples() {
        ...
        let mut params = WhirConfig::<EF, F, MyHash, MyCompress, MyChallenger>::new(
            num_variables,
            whir_params.clone(),
        );
        ...
        let dft = Radix2DFTSmallBatch::<F>::default();
        let committer = CommitmentWriter::new(&params);
        let witness = committer.commit(&dft, &mut proof, &mut challenger, polynomial);
