    ...
    F: Field,
{
    ...
    /// # Arguments
    ///
    /// - `proof`: The proof data the verifier reads.
    /// - `challenger`: The verifier's Fiat-Shamir state.
    ...
    ///
    #[allow(unreachable_pub)]
    pub fn parse_rf<EF, Challenger, const DIGEST_ELEMS: usize>(
        ...
        Challenger: FieldChallenger<F> + GrindingChallenger<Witness = F>,
    {
        ...
        for eval in proof.initial_ood_answers.iter().take(ood_samples) {
            ...
            let base_components = EF::flatten_to_base(vec![*eval]);
            challenger.observe_slice(&base_components);
    ...
    Challenger: FieldChallenger<F> + GrindingChallenger<Witness = F>,
{
    ...
    pub fn parse_commitment<const DIGEST_ELEMS: usize>(
        ...
        ParsedCommitment::<_, Hash<F, F, DIGEST_ELEMS>>::parse_rf(
            proof,
            challenger,
            self.num_variables,
...
mod tests {
    ...
    use crate::{
        ...
        whir::{
            DomainSeparator, EF, committer::writer_rf::CommitmentWriter,
    ...
    fn make_test_params<const DIGEST_ELEMS: usize>(
        ...
        WhirConfig<BinomialExtensionField<BabyBear, 4>, BabyBear, MyHash, MyCompress, MyChallenger>,
        SmallRng,
    ...
    fn test_commitment_roundtrip_with_ood() {
        ...
        let (params, mut rng, mut proof) = make_test_params::<8>(5, 3);
        ...
        let witness = committer.commit(&dft, &mut proof, &mut prover_challenger, polynomial);
    ...
    fn test_commitment_roundtrip_no_ood() {
        ...
        let (params, mut rng, mut proof) = make_test_params::<8>(4, 0);
        ...
        // Generate the prover challenger from the transcript.
        let mut rng = SmallRng::seed_from_u64(1);
        let mut prover_challenger = MyChallenger::new(Perm::new_from_rng_128(&mut rng));
    ...
    fn test_commitment_roundtrip_large_polynomial() {
        ...
        let (params, mut rng, mut proof) = make_test_params::<8>(10, 5);
        ...
        // Create prover challenger from the transcript.
        let mut rng = SmallRng::seed_from_u64(1);
        let mut prover_challenger = MyChallenger::new(Perm::new_from_rng_128(&mut rng));
        ...
        // Initialize verifier challenger from same seed.
        let mut rng = SmallRng::seed_from_u64(1);
        let mut verifier_challenger = MyChallenger::new(Perm::new_from_rng_128(&mut rng));
        ds.observe_domain_separator(&mut verifier_challenger);
    ...
    fn test_oods_constraints_correctness() {
        ...
        let (params, mut rng, mut proof) = make_test_params::<8>(4, 2);
        ...
        // Generate prover transcript state.
        let mut rng = SmallRng::seed_from_u64(1);
        let mut prover_challenger = MyChallenger::new(Perm::new_from_rng_128(&mut rng));
        ...
        // Generate verifier transcript state.
        let mut rng = SmallRng::seed_from_u64(1);
        let mut verifier_challenger = MyChallenger::new(Perm::new_from_rng_128(&mut rng));
        ds.observe_domain_separator(&mut verifier_challenger);
