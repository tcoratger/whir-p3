...
#[derive(Debug)]
pub struct CommitmentWriter<'a, EF, F, H, C, Challenger, Dft>(
...
impl<'a, EF, F, H, C, Challenger, Dft> CommitmentWriter<'a, EF, F, H, C, Challenger, Dft>
where
    F: TwoAdicField,
...
impl<EF, F, H, C, Challenger, Dft> Deref for CommitmentWriter<'_, EF, F, H, C, Challenger, Dft>
where
    F: TwoAdicField,
    EF: ExtensionField<F>,
    Dft: TwoAdicSubgroupDft<F>,
...
mod tests {
    ...
    fn test_basic_commitment() {
        ...
        let params = WhirConfig::<F, F, MyHash, MyCompress, MyChallenger, _>::new(
            num_variables,
            whir_params,
        );
        ...
        domainsep.commit_statement::<_, _, _, _, 8>(&params);
        domainsep.add_whir_proof::<_, _, _, _, 8>(&params);
    ...
    fn test_commitment_without_ood_samples() {
        ...
        let mut params = WhirConfig::<F, F, MyHash, MyCompress, MyChallenger, _>::new(
            num_variables,
            whir_params,
        );
