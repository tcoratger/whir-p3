    ...
    EF: ExtensionField<F>,
{
    ...
    // SYNC WITH EXTERNAL CHALLENGER AND VERIFY CONSISTENCY

    // Sample the same queries from the external challenger
    let mut queries_challenger = Vec::with_capacity(num_queries);
    ...
    if total_bits_needed <= max_bits_per_call {
        ...
        for _ in 0..num_queries {
            let query_bits = all_bits & mask;
            queries_challenger.push(query_bits % folded_domain_size);
            all_bits >>= domain_size_bits;
        }
    } else {
        let queries_per_batch = max_bits_per_call / domain_size_bits;
        ...
        if queries_per_batch >= 2 {
            ...
            while remaining > 0 {
                ...
                for _ in 0..batch_size {
                    let query_index = (all_bits & mask) % folded_domain_size;
                    queries_challenger.push(query_index);
                    all_bits >>= domain_size_bits;
                }
    ...
    queries_challenger.sort_unstable();
    queries_challenger.dedup();

    // Assert that both challengers produced the same query indices
    assert_eq!(
        queries, queries_challenger,
        "STIR query indices diverged between prover_state and external challenger"
    );

    Ok(queries)
}

/// WHIR STIR Query Sampler for Verifier: Generates query indices from a single challenger.
///
/// This is a simplified version for the verifier that doesn't need dual-system verification.
///
/// ## Parameters
/// - `domain_size`: Original evaluation domain size before folding
/// - `folding_factor`: Number of folding rounds (domain reduction = 2^folding_factor)
/// - `num_queries`: Target number of query indices to sample
/// - `challenger`: Challenger for deterministic randomness
///
/// ## Returns
/// Sorted, deduplicated vector of query indices in [0, folded_domain_size)
pub fn get_challenge_stir_queries_verifier<Challenger, F, EF>(
    domain_size: usize,
    folding_factor: usize,
    num_queries: usize,
    challenger: &mut Challenger,
