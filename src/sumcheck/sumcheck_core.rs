            ...
            .map(|point| {
                MultilinearPoint(
                    base_decomposition(point, 3, folding_factor)
                        .into_iter()
                        .map(|v| [F::ZERO, F::ONE, F::TWO][v as usize])
                        .collect(),
                )
            })
            .collect();
...
mod tests {
    ...
    fn test_sumcheck_core_initialization_three_variables() {
        ...
        // f(1,1,1) = 8
        let c1 = BabyBear::from_u64(1);
        let c2 = BabyBear::from_u64(4);
        let c3 = BabyBear::from_u64(3);
        let c4 = BabyBear::from_u64(7);
        let c5 = BabyBear::from_u64(2);
        let c6 = BabyBear::from_u64(6);
        let c7 = BabyBear::from_u64(5);
    ...
    fn test_sumcheck_core_with_equality_constraints() {
        ...
        let eq_weight = BabyBear::from_u64(2);

        // Given constraint point (1,0), compute eq_poly(X1, X2) at each binary point:
        let f_00 = eq_weight * BabyBear::from_u64(0); // f(0,0) += 0
        let f_01 = eq_weight * BabyBear::from_u64(0); // f(0,1) += 0
        let f_10 = eq_weight * BabyBear::from_u64(1); // f(1,0) += 2
        let f_11 = eq_weight * BabyBear::from_u64(0); // f(1,1) += 0
    ...
    fn test_compute_sumcheck_polynomial_with_equality_constraints() {
        ...
        let f_00 = BabyBear::ZERO; // f(0,0) unaffected
        let f_01 = BabyBear::ZERO; // f(0,1) unaffected
        let f_10 = BabyBear::TWO; // f(1,0) modified by the constraint
        let f_11 = BabyBear::ZERO; // f(1,1) unaffected
    ...
    fn test_compute_sumcheck_polynomial_with_multiple_equality_constraints() {
        ...
        let f_000 = BabyBear::ZERO;
        let f_001 = BabyBear::ZERO;
        let f_010 = BabyBear::from_u64(3); // Modified by constraint 2
        let f_011 = BabyBear::ZERO;
        let f_100 = BabyBear::ZERO;
        let f_101 = BabyBear::from_u64(2); // Modified by constraint 1
        let f_110 = BabyBear::ZERO;
        let f_111 = BabyBear::ZERO;
