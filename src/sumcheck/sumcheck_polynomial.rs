    ...
    F: Field,
{
    ...
    pub fn sum_over_boolean_hypercube(&self) -> F {
        ...
        // This is always called on a univariate polynomial from a sumcheck round.
        assert_eq!(
            self.n_variables, 1,
            "Only valid for univariate sumcheck polynomials"
        );
    ...
    pub fn evaluate_on_standard_domain(&self, point: &MultilinearPoint<F>) -> F {
        ...
        assert_eq!(
            self.n_variables, 1,
            "Only implemented for univariate polynomials"
        );

        let x = point[0];
        let h0 = self.evaluations[0];
