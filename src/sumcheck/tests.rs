...
// Sponge-based hash: absorbs 16 field elements, squeezes 8, with 8-element capacity.
type MyHash = PaddingFreeSponge<Perm, 16, 8, 8>;
// Merkle tree compression: truncates two 8-element digests into one via the permutation.
type MyCompress = TruncatedPermutation<Perm, 2, 8, 16>;
// Fiat-Shamir challenger: duplex sponge over BabyBear with width 16 and capacity 8.
type MyChallenger = DuplexChallenger<F, Perm, 16, 8>;
// Packed field type for SIMD-friendly evaluation (used by the Merkle tree MMCS).
type PackedF = <F as Field>::Packing;
// Merkle tree-based mixed-matrix commitment scheme, used for committing to polynomial evaluations.
...
// Builds a minimal ProtocolParameters instance for testing. The security parameters
// are intentionally low (32-bit security, no proof-of-work grinding) to keep tests fast.
// The MMCS (mixed-matrix commitment scheme) is constructed from the same Poseidon2
// permutation used elsewhere, ensuring a consistent cryptographic setup.
fn create_test_protocol_params(folding_factor: FoldingFactor) -> ProtocolParameters<MyMmcs> {
    // Use the same fixed seed as domainsep_and_challenger for reproducibility.
    let mut rng = SmallRng::seed_from_u64(1);
    let perm = Perm::new_from_rng_128(&mut rng);

    // Build the Merkle tree MMCS from the hash and compression functions.
    // The trailing `0` is the log of the packing ratio (no packing beyond the native width).
...
fn run_sumcheck_test(
    ...
    // Create protocol parameters and allocate the proof structure.
