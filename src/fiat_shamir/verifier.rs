...
/// verifier.
#[derive(Debug)]
pub struct VerifierState<'a, EF, F, Challenger, U, const DIGEST_ELEMS: usize>
where
    U: Unit,
    Challenger: CanObserve<U> + CanSample<U>,
{
    ...
    /// The proof data.
    pub(crate) proof_data: ProofData<EF, F, U, DIGEST_ELEMS>,
...
impl<'a, EF, F, Challenger, U, const DIGEST_ELEMS: usize>
    VerifierState<'a, EF, F, Challenger, U, DIGEST_ELEMS>
where
    U: Unit + Default + Copy,
    Challenger: CanObserve<U> + CanSample<U>,
    ...
    F: PrimeField64 + TwoAdicField,
{
    ...
    pub fn read_digest(&mut self) -> ProofResult<Hash<F, U, DIGEST_ELEMS>> {
...
//         let vs = VerifierState::<F, F, _, _, 32>::new(&ds, transcript, challenger);
...
//         ds.observe(3, "input");
//         let challenger = DummyChallenger::new();
//         let mut vs = VerifierState::<F, F, _, u8, 32>::new(&ds, b"abc", challenger);
//         let mut buf = [0u8; 3];
...
//         ds.observe(4, "fail");
//         let challenger = DummyChallenger::new();
//         let mut vs = VerifierState::<F, F, _, u8, 32>::new(&ds, b"xy", challenger);
//         let mut buf = [0u8; 4];
//         let res = vs.fill_next_units(&mut buf);
//         assert!(res.is_err());
//     }
...
//         ds.sample(4, "c");
//         let challenger = DummyChallenger::new();
//         let mut vs = VerifierState::<F, F, _, u8, 32>::new(&ds, b"abcd", challenger);
...
//         ds.observe(3, "byte");
//         let challenger = DummyChallenger::new();
//         let mut vs = VerifierState::<F, F, _, u8, 32>::new(&ds, b"xyz", challenger);
//         let mut out = [0u8; 3];
...
//         let challenger = DummyChallenger::new();
//         let mut prover =
//             domsep.to_verifier_state::<_, 32>(&[], Default::default(), challenger.clone());
