...
mod tests {
    ...
    fn test_absorb_zero_panics() {
        DomainSeparator::<EF4, F>::new("x").absorb(0, "label");
    }

    #[test]
    #[should_panic]
    fn test_label_with_separator_byte_panics() {
        DomainSeparator::<EF4, F>::new("x").absorb(1, "bad\0label");
    }

    #[test]
    #[should_panic]
    fn test_label_starts_with_digit_panics() {
        DomainSeparator::<EF4, F>::new("x").absorb(1, "1label");
    }

    #[test]
    fn test_merge_consecutive_absorbs_and_squeezes() {
        let mut ds = DomainSeparator::<EF4, F>::new("merge");
        ds.absorb(1, "a");
        ds.absorb(2, "b");
        ds.squeeze(3, "c");
        ds.squeeze(1, "d");
    ...
    fn test_parse_domsep_multiple_ops() {
        ...
        let ds = DomainSeparator::<EF4, F>::from_string(tag.to_string());
        let ops = ds.finalize();
    ...
    fn test_empty_operations() {
        let ds = DomainSeparator::<EF4, F>::new("tag");
        let ops = ds.finalize();
        assert!(ops.is_empty());
    }
    ...
    fn test_unicode_labels() {
        let mut ds = DomainSeparator::<EF4, F>::new("emoji");
        ds.absorb(1, "🦀");
        ds.squeeze(1, "🎯");
        let ops = ds.finalize();
    ...
    fn test_large_counts_and_labels() {
        ...
        let mut ds = DomainSeparator::<EF4, F>::new("big");
    ...
    fn test_malformed_tag_parsing_fails() {
        ...
        let ds = DomainSeparator::<EF4, F>::from_string(broken.to_string());
    ...
    fn test_round_trip_operations() {
        ...
        let ds2 = DomainSeparator::<EF4, F>::from_string(tag.to_string());
    ...
    fn test_squeeze_zero_count_panics() {
        DomainSeparator::<EF4, F>::new("proto").squeeze(0, "label");
    }

    #[test]
    #[should_panic]
    fn test_squeeze_label_with_null_byte_panics() {
        DomainSeparator::<EF4, F>::new("proto").squeeze(2, "bad\0label");
    }

    #[test]
    #[should_panic]
    fn test_squeeze_label_starts_with_digit_panics() {
        DomainSeparator::<EF4, F>::new("proto").squeeze(2, "1invalid");
    }
    ...
    fn test_challenge_scalars_babybear() {
        ...
        let mut domsep: DomainSeparator<F, F> = DomainSeparator::new("bb");
    ...
    fn test_challenge_scalars_quartic_ext_field() {
        ...
        // - "S" indicates squeezing in the domain separator
        let mut domsep: DomainSeparator<EF4, F> = DomainSeparator::new("ext2");
    ...
    #[test]
    fn test_add_ood() {
        let iop: DomainSeparator<F, F> = DomainSeparator::new("test_protocol");
    ...
    fn test_hint_is_parsed_correctly() {
        let mut ds = DomainSeparator::<EF4, F>::new("hint_test");
        ds.hint("my_hint");
        let ops = ds.finalize();
    ...
    fn test_hint_combined_with_absorb_and_squeeze() {
        let mut ds = DomainSeparator::<EF4, F>::new("combo");
        ds.absorb(1, "x");
        ds.hint("meta");
        ds.squeeze(2, "y");
        let ops = ds.finalize();
