...
/// Configuration parameters for challenge query generation.
#[derive(Clone, Debug)]
pub struct ChallengeQueryConfig {
    /// Threshold for switching between direct sampling and batch processing.
    /// When total_bits < batch_threshold, use direct sampling for better performance.
    /// When total_bits >= batch_threshold, use batch processing for efficiency.
    pub batch_threshold: usize,
    /// Maximum number of bits that can be safely requested in a single sample_bits call.
    /// This limit ensures compatibility with field constraints and prevents overflow.
...
impl Default for ChallengeQueryConfig {
    fn default() -> Self {
        Self {
            // Based on empirical benchmarking, direct sampling is faster for most practical cases
            batch_threshold: 10000,
            // Conservative limit to ensure (1 << bits) < F::ORDER_U64 for most fields
            max_bits_per_call: 30,
        }
    }
}

impl ChallengeQueryConfig {
    /// Creates a new configuration with custom parameters.
    #[must_use]
    pub const fn new(batch_threshold: usize, max_bits_per_call: usize) -> Self {
        Self {
            batch_threshold,
            max_bits_per_call,
        }
    ...
    /// Creates a configuration optimized for small-scale operations.
    #[must_use]
    pub const fn small_scale() -> Self {
        Self {
            batch_threshold: 5000,
            max_bits_per_call: 25,
        }
    }

    /// Creates a configuration optimized for large-scale operations.
    #[must_use]
    pub const fn large_scale() -> Self {
        Self {
            batch_threshold: 20000,
            max_bits_per_call: 35,
        }
    }
    ...
    pub const fn validate(&self) -> Result<(), &'static str> {
        if self.batch_threshold == 0 {
            return Err("batch_threshold must be greater than 0");
        }
        if self.max_bits_per_call == 0 {
            return Err("max_bits_per_call must be greater than 0");
        }
        if self.max_bits_per_call > 63 {
            return Err("max_bits_per_call must not exceed 63 to prevent overflow");
        }
        Ok(())
    }
}
